...
Value *Compiler::emitIfThenElse(std::unique_ptr<Expr> condition,
  ...
  // Free Floating blocks for now , we don't want to insert them until we finish
  // the if to handle nesting
...
Constant *Compiler::emitForLoop(const std::string &VarName,
...
Value *Compiler::emitCall(llvm::Function *F, std::vector<Value *> Args,
                          std::string name) {
  return Builder->CreateCall(F, Args, name);
}
Function *Compiler::emitPrototype(const std::string &name,
